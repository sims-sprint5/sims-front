      ...
      <template #cell-actions="{ item }">
        <div class="flex gap-2 justify-end">
          ...
          <button
            ...
            class="px-3 py-2 bg-success text-inverse text-sm font-medium hover:bg-success/90 rounded-lg transition-colors"
  ...
  <div class="md:hidden space-y-4">
    ...
    <div v-else v-for="item in reservations" :key="item.id" class="bg-surface rounded-lg border border-default p-4 shadow-sm">
      ...
      <div class="flex gap-2">
        <button
          @click="$emit('view-vehicle', item)"
          class="flex-1 p-2 bg-primary text-inverse text-sm font-medium hover:bg-primary-hover rounded-lg transition-colors flex items-center justify-center gap-2"
          :title="$t('common.view')"
        >
        ...
        <button
          v-if="!item.is_expired && getReservationDisplayStatus(item) !== 'completed'"
          @click="$emit('edit', item)"
          class="flex-1 p-2 bg-success text-inverse text-sm font-medium hover:bg-success/90 rounded-lg transition-colors"
        >
          {{ $t('reservations.buttons.expandButton') }}
        </button>
        <button
          v-if="canCancelReservation(item)"
          @click="$emit('delete', item)"
          class="flex-1 p-2 bg-danger text-inverse text-sm font-medium hover:bg-danger/90 rounded-lg transition-colors flex items-center justify-center gap-2"
          :title="$t('reservations.myReservations.cancel')"
        >
