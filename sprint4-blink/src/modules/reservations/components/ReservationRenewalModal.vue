      ...
      >
        <div class="bg-surface rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
          ...
          <div class="p-6 space-y-6">
            ...
            <!-- Reservation Summary -->
            <div class="space-y-3 rounded-lg bg-base p-4">
              <div class="space-y-1">
                <p class="text-xs font-medium uppercase text-muted">{{ $t('reservations.renewal.vehicle') }}</p>
                <p class="font-semibold text-main">{{ reservation.vehicle_name }}</p>
                <p class="text-xs text-muted">{{ reservation.license_plate }}</p>
              </div>

              <div class="pt-2">
                <p class="text-xs font-medium uppercase text-muted">{{ $t('reservations.renewal.currentEnd') }}</p>
                <p class="font-semibold text-main">{{ formatDateTime(reservation.end_at) }}</p>
              </div>
            ...
            <div class="space-y-2 border-t pt-4">
              <p class="text-sm font-medium text-main">{{ $t('reservations.renewal.proceedInfo') }}</p>
              <p class="text-xs text-muted">{{ $t('reservations.renewal.paymentInfo') }}</p>
            </div>
