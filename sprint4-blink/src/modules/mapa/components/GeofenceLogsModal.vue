  ...
  <CustomModal :show="open" :title="`${$t('mapa.viewLogs')} - ${geofence?.name}`" @close="$emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="logs.length === 0" class="text-center py-8">
        <p class="text-muted">{{ $t('mapa.noLogs') }}</p>
      </div>
        ...
        <table class="min-w-full border-collapse">
          <thead>
            <tr class="bg-surface-muted">
              <th class="px-4 py-2 text-left text-sm font-medium text-main">{{ $t('mapa.logs.vehicle') }}</th>
              <th class="px-4 py-2 text-left text-sm font-medium text-main">{{ $t('mapa.logs.eventType') }}</th>
              <th class="px-4 py-2 text-left text-sm font-medium text-main">{{ $t('mapa.logs.timestamp') }}</th>
              <th class="px-4 py-2 text-left text-sm font-medium text-main">{{ $t('mapa.logs.location') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="log in logs" :key="log.log_id" class="border-b hover:bg-base-dark">
...
const getEventTypeColor = (eventType: string): string => {
  ...
  return colors[eventType] || 'bg-surface-muted text-main'
}
</script>
