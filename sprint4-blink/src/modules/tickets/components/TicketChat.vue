    ...
    <div class="border-b pb-3 mb-3 px-4 sm:px-0">
      <div class="flex items-start justify-between gap-2 sm:gap-3">
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs sm:text-sm text-muted mt-0.5 truncate">{{ ticket.usuario_nombre }} &lt;{{ ticket.usuario_email }}&gt;</p>
        </div>
    ...
    <div class="border-t pt-3 sm:pt-4 px-4 sm:px-0">
      <form @submit.prevent="handleSubmit" class="space-y-2 sm:space-y-3">
        <textarea
          v-model="newMessage"
          rows="2"
          class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-default rounded-xl transition-all focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-sm"
