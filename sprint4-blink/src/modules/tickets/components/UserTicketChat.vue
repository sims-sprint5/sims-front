<template>
  <div class="flex flex-col h-[70vh] sm:h-[55vh] md:h-[480px] w-full">
    ...
    <div class="border-b pb-3 mb-3 px-4 sm:px-0">
      <div class="flex items-start justify-between gap-2 sm:gap-3">
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs sm:text-sm text-gray-500 mt-0.5 truncate">{{ formatDate(ticket.created_at) }}</p>
        </div>
    ...
    <div class="flex-1 overflow-y-auto space-y-3 sm:space-y-4 mb-3 sm:mb-4 px-2 sm:px-1" ref="messagesContainer">
      ...
      >
        <div v-if="!isOwnMessage(message)" class="shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-purple-100 border border-purple-200 flex items-center justify-center flex-shrink-0">
        ...
        <div :class="['w-full max-w-[92%] sm:max-w-[70%] md:max-w-[60%] flex flex-col', isOwnMessage(message) ? 'items-end' : 'items-start']">
          <span class="text-xs font-medium mb-0.5 sm:mb-1 px-1 line-clamp-1" :class="isOwnMessage(message) ? 'text-blue-700' : 'text-purple-700'">
    ...
    <div class="border-t pt-3 sm:pt-4 px-4 sm:px-0">
      <form @submit.prevent="handleSubmit" class="space-y-2 sm:space-y-3">
        <textarea
          v-model="newMessage"
          rows="2"
          class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-xl transition-all focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-sm"
